...
**Phase 3 ("Grow"), part 1 is built:** try-to-earn credits and structured feedback. Credits are called **V Coin** on screen (a mint pixel coin stamped with the logo's V, from `src/lib/pixel-coin.ts`); pages introduce them as credits first, then say V Coin. Everyone starts with 10 credits. Builders spend 2 credits per tester to put an app in the **Test & earn** queue (`/test`, linked from Browse; each app page explains Test & earn right under its description); people who open the app with Try it and leave feedback (would you use it, a rating, what worked, what confused you) earn 2 credits, plus 1 credit when the builder marks it helpful. Feedback is private to the tester and builder; app pages show the totals (testers, % who'd use it, average rating). Balance and history are at `/credits`.

**Phase 3 ("Grow") is built:**

- **Tester Passport:** every app you give feedback on stamps your passport. Ranks (Scout → Tester → Pro Tester → Trusted Tester) come from feedback given and feedback marked helpful, with perks built into the database: Testers earn 3 credits per paid feedback, Pro Testers can earn from 20 a day, Trusted Testers' feedback shows first to builders. 4 weeks in a row earns a 5-credit bonus. Profiles show the passport (rank, progress, a stamp per category, streak) and Home (and the Test & earn queue) show the month's top testers. Ranks never go down.
...
- **Boosts:** spend 10 credits a day (1, 3 or 7 days) to put an app in the Featured row with a *Boosted* label.
...
- **Pro (`/pro`):** $6 for 30 days, not a subscription. You get tries per day for the last 30 days (including how many came from sponsor cards), a pinned app on your profile, the Spotlight for 15 credits instead of 25 credits, and a Pro badge.
...
6. Optional, for tips, sponsorships, Pro and payouts: in [Stripe](https://dashboard.stripe.com), turn on **Connect** (Express accounts). Add a webhook destination for **your account** at `https://<your site>/api/stripe/webhook` with `checkout.session.completed` and `checkout.session.async_payment_succeeded`, and a second destination for **connected accounts** at the same URL with `account.updated`. Set `STRIPE_SECRET_KEY`, `STRIPE_WEBHOOK_SECRET` (the first destination's signing secret) and `STRIPE_CONNECT_WEBHOOK_SECRET` (the second's; optional, since payout setup is also read back when a builder returns from Stripe). `SUPABASE_SECRET_KEY` must be set too. Payments stay off unless all three are set. To test locally, use Stripe's test keys and `stripe listen --forward-to localhost:3000/api/stripe/webhook`.
...
9. Run `supabase/migrations/20261007000000_spotlight.sql`: the **Spotlight** replaces Boost (4 spots in the Featured row, 3 days each, 25 credits or 15 credits with Pro, first come first served with a line when they're all taken), and people can **buy credits** on `/credits` (25 for $5, 60 for $10, 150 for $20; needs the Stripe keys from the Earn setup). Credits can't be turned back into money, and they're sold on the website only.
10. Run `supabase/migrations/20261008000000_sponsor_packages.sql` for sponsorship packages, then `20261009000000_review_fixes.sql` (enforces the open-package limit at checkout and payment, keeps Boosts that are still running, and lets the SQL Editor give out `@methodv` on a fresh setup). Run the migration files in order, oldest first. Each is safe to run twice in a row; don't re-run an older file after newer ones (a newer file may have replaced what it sets up).
