...
export async function GET() {
  ...
  if (isSupabaseConfigured) {
    ...
    // Photos, social handles and polls are from the newest migrations, so they show the database is up to date.
    const [{ error: behindProfiles }, { error: behindQa }] = await Promise.all([
    ...
    const latest = behindProfiles ?? behindQa;
