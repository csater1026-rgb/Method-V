...
// 30 days of tries as simple bars; the darker part came from sponsor cards.
export function StatsChart({ stats, label }: { stats: AppStat[]; label: string }) {
  ...
  return (
    <figure aria-label={label}>
      <div className="flex h-36 items-end gap-[3px]" role="img" aria-label={`${total} tries in 30 days`}>
        {stats.map((s) => (
      ...
      <figcaption className="mt-2 flex flex-wrap gap-x-4 font-mono text-xs text-muted">
        <span>
          <strong className="text-ink">{total}</strong> tries in 30 days
        </span>
        <span>
